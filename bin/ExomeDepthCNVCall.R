...
out <- args$out #  "ExomeDepth_CNV_calls.txt"
# Optional bed file. If not given, full exome is used.
if (is.null(args$bed_file)) {
    data(exons.hg19)
    bed.file <- exons.hg19
    include.chr <- TRUE
} else {
    bed.file <- read.table(paste(args$bed_file), sep="\t")
    colnames(bed.file)<-c("chromosone","start","end","gene","name")
    include.chr <- FALSE
}
# Count BAMs
ExomeCount <- getBamCounts(bed.frame = bed.file, bam.files = bams, include.chr = include.chr, referenceFasta = ref_genome) # exons.hg19, or bed withOUT "chr".

# Filter bad samples (good = at least 20 amplicons with > 100 reads)
good_cols <- (colSums(ExomeCount > 100) > 20)
good_cols[1:5] <- TRUE # 1st 5 cols are not samples, always should be kept (chromosome      start        end       exon         GC)
ExomeCount <- ExomeCount[,good_cols]

# export counts:
write.table(ExomeCount, file=counts_out,sep="\t",quote=F,row.names=F,col.names=T)

# ExomeCount <- getBamCounts(bed.frame = bed.file, bam.files = bams, include.chr = FALSE, referenceFasta = ref_genome) # Bed file, if it has "chr"
sample.names <- names(ExomeCount)[-(1:5)] # columns with sample names
cnv.calls = NULL
refs<-list()
models<-list()
...
for(i in 1:length(sample.names)){#for each sample:
    ...
    my.choice <- select.reference.set (test.counts = my.test,reference.counts = my.reference.set,
        bin.length = (ExomeCount$end - ExomeCount$start)/1000) #from the reference set, selects correlated samples to be used.
    selected_refs <- my.choice$reference.choice
    my.matrix <- as.matrix( ExomeCount[, selected_refs, drop = FALSE])             #places selected, correlated samples into a matrix
    my.reference.selected <- apply(X = my.matrix,MAR = 1,FUN = sum)#sums the selected samples across each exon
    # my.reference.selected <- apply(X = my.matrix,MAR = 1,FUN = mean)#sums the selected samples across each exon
...
# Breaking same gene calls here, not sure if formatting is right; It seems to expect gene names, not exons.
...
write.table(cnv.calls_ids_out,file=out,sep="\t",quote=F,row.names=F,col.names=T)

write.csv(do.call("rbind",list(unlist(refs))),file=paste(out,".refs.txt",sep=""))
# save(refs, file=paste(out,"refs.Rdata",sep=""))
